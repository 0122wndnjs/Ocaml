...
(* This is the function get_time that prints out the timestamps. To do this, I used Unix.time(). And I declared some variables
that needs to represent the time. Seconds, minutes, hours, day, month, nyears. For years, I set the standard as 1900 and by
adding nyear, it shows the year. Lastly, I used string_of_int for every variables. Between date, I used "/" to differentiate
month/day/year and ":" for time so that hours:minutes:seconds. So the final format of the output would be MM/DD/YYYY_HH/mm/SS. *)
let rec get_time = let time = Unix.time() in
  let {Unix.tm_sec = seconds; tm_min = minutes; tm_hour = hours; tm_mday = day; tm_mon = month; tm_year = nyear;} = Unix.localtime time in
  let year = nyear + 1900 in
  "("^(string_of_int month)^"/"^(string_of_int day)^"/"^(string_of_int year)^"_"^(string_of_int hours)^":"^(string_of_int minutes)^":"^(string_of_int seconds)^")"
...
to show to every user except for sender. One thing that is different with chatServer.ml is that since I wrote a function to
update the time, which is get_time, I called the function to be printed out. In the second line, I wrote (get_time^" "^sender^": ")
so that whenever users write something on the chat, it will print out the time and the user's name. *)
let rec broadcast sender msg =  let test s =  match s with
  (name,output) -> if name <> sender then (Lwt_io.fprint output (get_time^" "^sender^": ")
