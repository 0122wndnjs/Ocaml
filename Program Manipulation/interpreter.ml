...
if Array.length Sys.argv < 2 then () else
  ...
  (* Parse the program...*)
  (* I modified the last expression to parse_pos that I used to throw the exception
  in the parser.ml. *)
  let progExpr = progString |> wordlist |> tokens |> parse_pos in
  ...
  (* calling const_fold on progExpr. *)
  let x = (const_fold progExpr) in
  (* calling unused_def_elim on the result of const_fold. So I used in right after
  the const_fold calling since it is the result of const_fold. *)
  let y = (unused_def_elim x) in
  (* Run the program (optimized) program. *)
  let _ = eval progExpr [] in () (* change progExpr to whatever name you bound to (unused_def_elim (const_fold progExpr)) above *)
