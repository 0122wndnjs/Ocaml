type expr =
  ...
  | Apply of expr * expr
  | Print of expr
  | Readint  (* Added new variant Readint to expr type *)
...
let rec eval exp state = match exp with
  ...
  (* When encounters a Readint, read integer using ocaml function read_int() *)
...
let rec typeof exp env = match exp with
  | Readint -> Int (* Modifying type checking function, to infer the correct type of Readint value in expression *)
  | Const _ -> Int
  | Boolean _ -> Bool
