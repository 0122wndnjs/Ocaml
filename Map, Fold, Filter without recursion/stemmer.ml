...
let match_rule word ((num,orig,_,min_root) : (int * string * string * int)) =
  let orig_len = String.length orig in
  ...
  in
    if word_len > orig_len then
      let word_end = (String.sub word (word_len - orig_len) orig_len) in
      let word_root = (String.sub word 0 (word_len - orig_len)) in
        if word_end = orig &&
...
let apply_rule word ((_,orig,rep,_) : int * string * string * int)=
  let orig_len = String.length orig in
  let word_len = String.length word in
  let orig_word = word in
  let new_word = (String.sub word 0 (word_len - orig_len)) ^ rep in
...
let stem in_word =
  if String.length in_word < 2 then "" else
  ...
  if 0 = (String.compare "'s" (String.sub word ((String.length word)-2) 2)) then
    word
  else
    aux word all_rules
;;
